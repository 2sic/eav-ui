/**
 * spm NOTE: ace doesn't seem to have a of importing types without importing
 * entire ace-builds package (bundling ace scripts with the project).
 * On top of that, types can be outdated. This is a copy of the model from https://github.com/ajaxorg/ace-builds
 */
...
export interface Ace {
  ...
  createEditSession(text: Document | string, mode: SyntaxMode): EditSession;
  VirtualRenderer: {
    new(container: HTMLElement, theme?: string): VirtualRenderer;
  };
  EditSession: {
    new(text: string | Document, mode?: SyntaxMode): EditSession;
  };
  UndoManager: {
    new(): UndoManager;
  };
