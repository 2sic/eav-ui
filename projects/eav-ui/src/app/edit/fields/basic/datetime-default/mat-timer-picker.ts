...
export class MyMatTimepickerInput<D> implements ControlValueAccessor, Validator, OnDestroy {
  ...
  private _registerTimepicker(): void {
    effect(() => {
      const timepicker = this.timepicker();
      // @ts-expect-error: We intentionally register our custom input type
