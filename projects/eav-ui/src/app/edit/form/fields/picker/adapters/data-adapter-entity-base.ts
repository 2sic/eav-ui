...
export abstract class DataAdapterEntityBase extends DataAdapterBase {
  ...
  protected contentType = computed(() => this.contentTypeMaskLazy()?.signal() ?? '');
  private createEntityTypes = signal<string>('');
  ...
  init(callerName: string): void {
    ...
    const typeMaskFromSettings = computed(() => this.settings().EntityType, SignalHelpers.stringChanges);
    const createTypesFromSettings = computed(() => this.settings().CreateTypes, SignalHelpers.stringChanges);
