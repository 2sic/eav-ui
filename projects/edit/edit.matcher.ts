...
export function editRoot(url: UrlSegment[]): UrlMatchResult {
  ...
  const hasUpdateFieldId = !!url[4] && url[4].path === 'update' && !!url[5];
  const match: UrlMatchResult = {
    consumed: url.slice(0, (hasExpandedFieldId || hasUpdateFieldId) ? 6 : 4),
    posParams: {
      zoneId: url[0],
      appId: url[1],
      items: url[3],
      ...(hasExpandedFieldId && { expandedFieldId: url[5] }),
      ...(hasUpdateFieldId && { updateFieldId: url[5] }),
    }
  };
  return match;
}

/** 'edit/:items' or 'edit/:items/details/:expandedFieldId' or 'edit/:items/update/:updateFieldId' */
export function edit(url: UrlSegment[]): UrlMatchResult {
  ...
  const hasUpdateFieldId = !!url[2] && url[2].path === 'update' && !!url[3];
  const match: UrlMatchResult = {
    consumed: url.slice(0, (hasExpandedFieldId || hasUpdateFieldId) ? 4 : 2),
    posParams: {
      items: url[1],
      ...(hasExpandedFieldId && { expandedFieldId: url[3] }),
      ...(hasUpdateFieldId && { updateFieldId: url[3] }),
    }
  };
  return match;
}
